...
inline std::string odin_frame(const std::string& frame) {
    return g_frame_prefix.empty() ? frame : g_frame_prefix + "/" + frame;
}
#ifdef ROS2
...
public:
    ...
    void publishImu(imu_convert_data_t *stream) {
        ...
        #endif
        imu_msg.header.frame_id = odin_frame("imu");
...
void process_pair(const ImageConstPtr &rgb_msg, const PointCloud2ConstPtr &pcd_msg)
{
    ...
    if (input_image_height > 0 && input_image_width > 0) {
        ...
        output_msg.header.frame_id = odin_frame("lidar");
...
void publishIntensityCloud(capture_Image_List_t* stream, int idx)
{
    ...
    // Set message header
    msg->header.frame_id = odin_frame("lidar");
    #ifdef ROS2
...
void publishGrayUInt8(capture_Image_List_t *stream, int idx) {
    ...
    #endif
    msg.header.frame_id = odin_frame("map");
...
void publishRgb(capture_Image_List_t *stream) {
    ...
    void publishPC2XYZRGBA(capture_Image_List_t* stream, int idx) 
    {
                ...
                msg.header.frame_id = odin_frame("odom");
    ...
    void publishOdometry(capture_Image_List_t* stream, OdometryType odom_type, bool show_path, bool show_camerapose) {
            ...
            msg.header.frame_id = odin_frame("odom");
            msg.child_frame_id = odin_frame("imu");
            ...
            switch(odom_type) {
                case OdometryType::STANDARD:
                    ...
                    if (getRosNodeControl()->sendOdomBaseLinkTF()) {
                        ...
                        transformStamped.header.frame_id = odin_frame("odom");
                        transformStamped.child_frame_id = odin_frame("imu");
                        ...
                        // Publish til (imu -> lidar) and wc (odom -> camera_0)
                        {
                            ...
                            if (has_cached_extrinsics_) {
                                ...
                                tf_til.header.frame_id = odin_frame("imu");
                                tf_til.child_frame_id = odin_frame("lidar");
                                ...
                                tf_wc.header.frame_id = odin_frame("odom");
                                tf_wc.child_frame_id = odin_frame("camera_0");
                ...
                case OdometryType::TRANSFORM:
                    ...
                    transformStamped.header.stamp = msg.header.stamp;
                    transformStamped.header.frame_id = odin_frame("odom");
                    transformStamped.child_frame_id = odin_frame("map");
                    transformStamped.transform.translation.x = msg.pose.pose.position.x;
                    transformStamped.transform.translation.y = msg.pose.pose.position.y;
                    transformStamped.transform.translation.z = msg.pose.pose.position.z;
                    transformStamped.transform.rotation.x = msg.pose.pose.orientation.x;
