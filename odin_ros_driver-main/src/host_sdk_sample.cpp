...
std::string g_target_serial = "";
std::string g_frame_prefix = "";
std::string g_connected_serial = "";
...
static void lidar_device_callback(const lidar_device_info_t* device, bool attach)
{
    ...
    if(attach == true) {
        std::string device_serial(reinterpret_cast<const char*>(device->serial));
        std::string device_model(reinterpret_cast<const char*>(device->model));
        #ifdef ROS2
            RCLCPP_INFO(rclcpp::get_logger("device_cb"),
                        "Hardware connected: serial=%s model=%s initial_state=%u",
                        device_serial.c_str(), device_model.c_str(), device->initial_state);
        #else
            ROS_INFO("Hardware connected: serial=%s model=%s initial_state=%u",
                     device_serial.c_str(), device_model.c_str(), device->initial_state);
        #endif

        if (!g_target_serial.empty() && device_serial != g_target_serial) {
            #ifdef ROS2
                RCLCPP_INFO(rclcpp::get_logger("device_cb"),
                            "Ignoring device serial=%s because target_serial=%s",
                            device_serial.c_str(), g_target_serial.c_str());
            #else
                ROS_INFO("Ignoring device serial=%s because target_serial=%s",
                         device_serial.c_str(), g_target_serial.c_str());
            #endif
            return;
        }
        ...
        #ifdef ROS2
            RCLCPP_INFO(rclcpp::get_logger("device_cb"), "Starting software connection...");
        #else
            ROS_INFO("Starting software connection...");
        #endif
	...
	std::string config_dir = "";
        ...
        g_connected_serial = device_serial;
        ...
        std::string device_serial(reinterpret_cast<const char*>(device->serial));
        if (!g_connected_serial.empty() && device_serial != g_connected_serial) {
            #ifdef ROS2
                RCLCPP_INFO(rclcpp::get_logger("device_cb"),
                            "Ignoring detach for serial=%s; this instance is bound to serial=%s",
                            device_serial.c_str(), g_connected_serial.c_str());
            #else
                ROS_INFO("Ignoring detach for serial=%s; this instance is bound to serial=%s",
                         device_serial.c_str(), g_connected_serial.c_str());
            #endif
            return;
        }
...
int main(int argc, char *argv[])
{
    ...
    std::string ros2_config_file = node->declare_parameter<std::string>("config_file", "");
    g_target_serial = node->declare_parameter<std::string>("target_serial", "");
    g_frame_prefix = node->declare_parameter<std::string>("frame_prefix", "");
    g_command_file_path = node->declare_parameter<std::string>("command_file", "/tmp/odin_command.txt");
    RCLCPP_INFO(node->get_logger(),
                "Instance parameters: target_serial=%s frame_prefix=%s command_file=%s config_file=%s",
                g_target_serial.c_str(),
                g_frame_prefix.c_str(),
                g_command_file_path.c_str(),
                ros2_config_file.c_str());
    ...
    try {
        ...
        #ifdef ROS2
            if (!ros2_config_file.empty()) {
                config_file = ros2_config_file;
